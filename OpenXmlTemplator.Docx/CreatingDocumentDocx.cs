namespace OpenXmlTemplator.Docx
{
    ...
    public static class CreatingDocumentDocx
    {
        ...
        public static async Task<byte[]> MergedDocuments(DocumentModelDocx docxTemplatorModels, CancellationToken token = default)
        {
            using (MemoryStream docxStream = new())//Поток для итогового документа
            {
                ...
                using (ZipArchive docx = new(stream: docxStream, mode: ZipArchiveMode.Update))
                {
                    ...
                    using (Stream documentXmlStream = await documentXmlEntry.OpenAsync(token))
                    {
                        ...
                        foreach ((string documentName, KeyWordsHandlerModelDocx keyWords) in docxTemplatorModels.Documents)
                        {
                            ...
                            BodyRecursiveReplace(newBody, keyWords, docxTemplatorModels);
                        ...
                        using (MemoryStream docxStream = new())//поток для записи docx файла в rar архив
                        {
                            ...
                            using (ZipArchive docx = new(stream: docxStream, mode: ZipArchiveMode.Update, leaveOpen: true))
                            {
                                ...
                                using (Stream documentXmlStream = await documentXmlEntry.OpenAsync(token))
                                {
                                    ...
                                    BodyRecursiveReplace(body, keyWords, docxTemplatorModels);
        ...
        /// <param name="docxTemplatorModels"></param>
        private static void BodyRecursiveReplace(XElement body, KeyWordsHandlerModelDocx keyWords, DocumentModelDocx docxTemplatorModels)
        {
        ...
        private static async Task DocumentSave(Stream documentXmlStream, XDocument document, CancellationToken token = default)
        {
